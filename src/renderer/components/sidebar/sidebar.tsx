...
export function Sidebar() {
    ...
    const [sidebarWidth, setSidebarWidth] = useState(initialSidebarWidth ? Number(initialSidebarWidth) : SIDEBAR_INITIAL_WIDTH);

    const location = useLocation();

    const {
        game: gameDownloading, progress
    } = useDownload();
    ...
    const isDownloading = library.some((game) => ["downloading", "checking_files", "downloading_metadata"].includes(game.status));
    ...
    const handleFilter: React.ChangeEventHandler<HTMLInputElement> = (event) => {
        setFilteredLibrary(library.filter((game) => game.title.toLowerCase().includes(event.target.value.toLocaleLowerCase())));
    };
    ...
    const getGameTitle = (game: Game) => {
        ...
        if (gameDownloading?.id === game.id) {
            const isVerifying = ["downloading_metadata", "checking_files"].includes(gameDownloading?.status);
        ...
        >
            ...
            >
                {window.electron.platform === "darwin" && (
                    <h2 style={{ marginBottom: SPACING_UNIT }}>grace</h2>
                )}
