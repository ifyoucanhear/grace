...
const getGOGGame = async (url: string) => {
    ...
    const $downloadButton = window.document.querySelector(".download-btn:not(.lightweight-accordion *)") as HTMLAnchorElement;
...
export const getNewGOGGames = async (existingRepacks: Repack[] = []) => {
    try {
        const data = await requestWebPage("https://freegogpcgames.com/a-z-games-list/");
