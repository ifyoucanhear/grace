...
export const request1337x = async (path: string) => requestWebPage(`https://1337xx.to${path}`);

/* todo: $a muitas vezes será nulo */
...
export const getNewRepacksFromUser = async (
    ...
    const newRepacks = repacks.filter(
        (repack) => repack.uploadDate && !existingRepacks.some(
            (existingRepack) => existingRepack.title === repack.title
        )
    );

    if (!newRepacks.length)
        return;
